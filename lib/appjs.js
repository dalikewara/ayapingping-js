 ...
 * @param {string|object|number|boolean} config - plugin config
 * @public
 */

appjs.load = function (name, config) {
  this.__proto.__stack.push({
    type: 'plugin',
    engine: 'appjs',
    name: name,
...
function singlePluginStack(stack, resolve) {
  ...
  (async function () {
    ...
    await new Promise(function (resolve, reject) {
      ...
      var pth = null;
      pluginPaths.plugins.map(function(nm) {
        if (nm === name) {
          pth = path.join(pluginPath, name);
        }
      });
      pluginPaths.libs.map(function (nm) {
        if (!pth) {
          if (nm === name) {
...
function singleRouteStack(stack, resolve) {
  ...
  if (stack.engine === 'express') {
    stack.callback(proto.__APP, app);
    resolve();
      ...
      if (typeof stack.middleware === 'string' ||
        ...
        if (middlewareMethod) {
          f.push(function (req, res, next) {
            ...
            )[middlewareMethod](proto.__APP, req, res, next);
          });
        } else {
          f.push(function (req, res, next) {
            require(path.join(middlewarePath, middlewareName)
            )(proto.__APP, req, res, next);
          });
        }
        if (controllerMethod) {
          app[stack.method](stack.endpoint,
            ...
            function (req, res, next) {
              ...
              )[controllerMethod](proto.__APP, req, res, next);
            }
          );
        } else {
          app[stack.method](stack.endpoint,
...
function getLoadedPlugins(resolve, reject) {
  var len = Object.keys(appjs.__proto.__APP).length;
