...
/**
 * Initialize & create app methods also initialize express app.
 * 
 * @private
 */
...
/**
 * Bind application-level middleware to an instance of the express object
 * May get error if not initialized, use try catch
 * @param {Object|function} application - application to binding
 * @public
 */
appjs.bindExpress = function(application) {
  if (!this.__proto.hasOwnProperty('__app') || this.__proto.__app == null
      || !this.__proto.__app || this.__proto.__app === {}) {
    throw Error("Express object has not been initialized");
  }
  this.__proto.__app.use(application);
}
...
function singlePluginStack(stack, resolve) {
  ...
  (async function () {
    ...
    await new Promise(function (resolve, reject) {
      ...
      if (require.cache[require.resolve(pth)]) delete require.cache[require.resolve(pth)];
      require(pth)(
        appjs.__proto.__app,
        getPublicProto(),
